...
It type-checks and tests independently and isn't wired into `agila`'s
navigation yet (no `android`/`ios`/entry point of its own beyond the
`ExampleUsage` demo). Chalmers-branded UI primitives (`ChalmersText`,
`ChalmersButton`, theme tokens) are **not** copied in here - every
component imports them directly from the shared `kar-ui-kit` package
(`file:../kar-ui-kit`), the same one `agila` uses. `i18n/translations.ts`
still stays local, since it only carries this module's own `studyRooms*`
keys, not `agila`'s unrelated evaluation copy.
